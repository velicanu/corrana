...
now="$3_$4_$5_$6_$7_$8_$9_$(date +"%Y_%m_%d__%H_%M_%S")"
...
cp $2 $now
cp runcorr.sh $now
cat runcorr.condor | sed "s@log_flag@$now@g" | sed "s@dir_flag@$PWD/$now@g" | sed "s@user_flag@$USER@g" |  sed "s@arglist@$1 $2 $3 $4 $5 $6 $7 $8 $9@g" | sed "s@transfer_filelist@$2,runcorr.exe@g" | sed "s@njobs@$njobs@g" > $now/runcorr.condor
